...
const categories = [
  ...
  { name: 'mobile-accessories', url: 'https://dummyjson.com/products/category/mobile-accessories' },
  { name: 'smartphones', url: 'https://dummyjson.com/products/category/smartphones' },
...
(async function seed() {
  try {
    ...
    for (const category of categories) {
      console.log(`Fetching category: ${category.name} -> ${category.url}`);
      try {
        const items = await fetchCategory(category.url);
        items.forEach(item => { if (!item.category) item.category = category.name; });

        const result = await upsertProducts(items, category.name);
        const upserted = result.upsertedCount || result.nUpserted || 0;
        const modified = result.modifiedCount || result.nModified || 0;

        console.log(`Category '${category.name}': fetched=${items.length}, upserted=${upserted}, modified=${modified}`);
        totalInserted += items.length;

        await new Promise(r => setTimeout(r, Number(process.env.API_SEED_TIMEOUT_MS || 200)));
      } catch (error) {
        console.log(`Failed to fetch category ${category.name}`, error.message);
      }
    }
